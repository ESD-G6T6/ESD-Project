...
def receiveScooter():
    ...
    channelqueue = channel.queue_declare(queue="parkingLot", durable=True) # 'durable' makes the queue survive broker restarts so that the messages in it survive broker restarts too
    queue_name = channelqueue.method.queue
    channel.queue_bind(exchange=exchangename, queue=queue_name, routing_key='parkingLot.scooterInfo') # bind the queue to the exchange via the key
...
def processScooter(scooter):
    print("Updating scooter information:")
    print(scooter)

    if "scooterID" in scooter:
        r = requests.put(parkingLotURL + str(scooter['parkingLotID']))
        print(r.text, r.status_code)

    # update parking lot db here
